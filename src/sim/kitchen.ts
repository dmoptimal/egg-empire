...
/**
 * Where table `slot` sits on the restaurant floor. Slots 0-3 are the front
 * row, 4-7 the staggered back row (the render supplies each row's y).
 */
export const customerSlotX = (s: SimState, slot: number): number =>
  Math.min(46 + (slot >= 4 ? 44 : 0) + (slot % 4) * 88, s.layout.w - 48);
